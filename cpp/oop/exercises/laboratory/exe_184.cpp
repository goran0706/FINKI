...
public:
    Flight() : distance(0), passengerNr(0) {
    }

    Flight(const char *flightNr, const char *destination, const int distance, const int passengerNr) {
        strcpy(this->flightNo, flightNr);
    ...
    Flight(const Flight &other) {
        strcpy(this->flightNo, other.flightNo);
        strcpy(this->destination, other.destination);
        this->distance = other.distance;
        this->passengerNr = other.passengerNr;
    }
    ...
    Flight &operator=(const Flight &other) {
        ...
        passengerNr = other.passengerNr;
        return *this;
    }

    ~Flight() = default;

    [[nodiscard]] double fuelCost() const {
        return distance * 2.5 + passengerNr * 0.8;
    }

    void print() const {
        cout << flightNo << " -> " << destination << "\n";
        cout << "Distance: " << distance << " km" << "\n";
        cout << "Passengers: " << passengerNr << "\n";
        cout << "Fuel Cost: " << fuelCost() << "\n";
    }
};


class CharteredFlight : public Flight {
protected:
    char company[50]{};
    double leasePrice;
    bool isPrivate;
...
public:
    CharteredFlight() : leasePrice(0), isPrivate(false) {
    }

    CharteredFlight(const Flight &obj, const char *company, const double leasePrice,
                    const bool isPrivate) : Flight(obj) {
    ...
    CharteredFlight(
        const char *flightNr, const char *destination, const int distance, const int passengerNr,
        const char *company, const double leasePrice, const bool isPrivate)
        : Flight(flightNr, destination, distance, passengerNr) {
        strcpy(this->company, company);
        this->leasePrice = leasePrice;
        this->isPrivate = isPrivate;
    }

    CharteredFlight &operator=(const CharteredFlight &other) {
        if (this == &other) return *this;
        Flight::operator =(other);
        leasePrice = other.leasePrice;
        isPrivate = other.isPrivate;
        return *this;
    }

    [[nodiscard]] double charterCost() const {
        if (isPrivate) {
            return fuelCost() * 3.0 + leasePrice;
        }
        return fuelCost() * 1.5 + leasePrice;
    }

    [[nodiscard]] static double fuelCost() {
        return 2;
    }

    void print() const {
        cout << "Company: " << company << "\n";
        cout << "Charter Price: " << leasePrice << "\n";
        cout << "Private: " << isPrivate << "\n";
        cout << "Charter Cost: " << charterCost() << "\n";
        cout << "Total Cost: " << fuelCost() << "\n";
    }
};
