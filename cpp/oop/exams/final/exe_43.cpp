/*
    ...
    Сите податоци во класата треба да се приватни. Сезонскиот попуст е ист за сите концерти и може да се менува од страна
    на менаџерите во зависност од сезоната. За таа цел да се обезбеди функција за менување на сезонскиот попуст.
    ...
    Доколку не е пронајден концертот, функцијата враќа false.
...
public:
    ...
    static void setSezonskiPopust(double x) {
        sezonski_popust = x;
    }
...
public:
    ElektronskiKoncert(char *naziv = "", char *lokacija = "", double cenaBilet = 0, char *imeDJ = nullptr,
                       double vremetraenje = 0, bool dnevna = false)
    ...
    double cena() override {
        double konecna = getCenaBilet() - getCenaBilet() * getSezonskiPopust();
        if (vremetraenje > 7) {
            konecna += 360;
...
void najskapKoncert(Koncert **koncerti, int n) {
    ...
    cout << "Elektronski koncerti: " << countElektronski << " od vkupno " << n << endl;
}
