// Да се креира класа SkiLift во која се чуваат податоци за името на ски лифтот (низа од 15 знаци), максимален број на корисници што може да опслужи на едно возење, дали е пуштен во функција.
//
// Потоа да се креирa класа SkiCenter во која се чуваат податоци за името на скијачкиот центар, држава во која се наоѓа (низи од 20 знаци), низа од ски лифтови (макс 20) што ги има и број на ски лифтови.
//
...
void najgolemKapacitet(skiCenter *sc, int n) {
    ...
    for (i = 0; i < n; i++) {
        tmp = kapacitet(sc[i]);
        if ((tmp > max) || (tmp == max && sc[i].br_lifts > sc[max_ind].br_lifts)) {
            max = tmp;
            max_ind = i;
        }
    }
