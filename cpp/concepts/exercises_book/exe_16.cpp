// 3. Write a program that asks the user to enter his or her first name and then last name, and that then constructs,
// stores, and displays a third string, consisting of the user’s last name followed by a comma, a space, and first name.
// Use char arrays and functions from the cstring header file. A sample run could look like this:
// Enter your first name: Flip
// Enter your last name: Fleming
// Here’s the information in a single string: Fleming, Flip

#include <cstring>
#include <iostream>


int main() {
    using namespace std;

    const int arraySize = 30;
    char firstName[arraySize];
    char lastName[arraySize];
    char fullName[arraySize * 2];

    cout << "Enter your first name: ";
    cin.getline(firstName, arraySize);

    cout << "Enter your last name: ";
    cin.getline(lastName, arraySize);

    strcpy(fullName, lastName); // copy lastName into fullName - works better with uninitialized strings
    strcat(fullName, ", "); // append ", "
    strcat(fullName, firstName); // append firstName

    cout << "Here’s the information in a single string: " << fullName << endl;

    return 0;
}
